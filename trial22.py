...
def load_panel(sector: str) -> pd.DataFrame:
    candidates = [
        Path(f"/mnt/data/{sector}/yt_brand_daily_panel.csv"),
        Path(f"./{sector}/data/yt_brand_daily_panel.csv"),
        Path(f"../{sector}/data/yt_brand_daily_panel.csv"),
        Path(f"{sector}/data/yt_brand_daily_panel.csv"),
    ]
    path = next((p for p in candidates if p.exists()), None)
    if path is None:
        st.error(f"Could not find yt_brand_daily_panel.csv for sector '{sector}'. Expected under {sector}/data or /mnt/data/{sector}.")
        st.stop()
...
with st.sidebar:
    # Sector selector (affects file paths)
    sector = st.selectbox(
        "Sector / 섹터",
        options=["beauty", "fnb", "kpop"],
        index=0,
        help="Choose which sector's dataset to load (expects <sector>/data/yt_brand_daily_panel.csv)."
    )

    # Load panel for the selected sector
    panel = load_panel(sector)
...
with st.sidebar:
    ...
    # If sector changed, reset brand selections to the new defaults
    if st.session_state.get("active_sector") != sector:
        st.session_state["active_sector"] = sector
        st.session_state["sel_brands"] = default_brands
        st.session_state["confirmed_brands"] = default_brands
        # Also reset verification brand so it aligns with the new sector
        if "verify_brand" in st.session_state:
            del st.session_state["verify_brand"]
...
# Footer
if lang == "English":
    st.caption(f"Data: {sector}/data/yt_brand_daily_panel.csv. Engagement = likes + comments. Timestamps treated as UTC. Start date filtered at 2025-09-21.")
else:
    st.caption(f"데이터: {sector}/data/yt_brand_daily_panel.csv. 참여 = 좋아요+댓글. 모든 시각은 UTC 기준입니다. 시작일은 2025-09-21로 필터링되었습니다.")