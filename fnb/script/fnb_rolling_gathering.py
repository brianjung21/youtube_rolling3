...
API_KEYS = [
    ...
    "AIzaSyAPoMTive8VCnoF59APWtM1ssrpEpOJjwE"
]
_current_key_idx = 0
...
KEYWORDS_PATH = Path('/Users/sanghoonjung/PycharmProjects/PythonProject/youtube_rolling3/fnb/files/fnb_keywords_sample.csv')
...
LANG_HINTS = ["en"]  # prefer Korean + English for discovery
...
CONTINUE_ON_QUOTA_ERRORS = False  # Stop the run when all keys hit quota/rate limits

# --- Discovery strategy tweaks (aligned with K-pop/F&B scripts) ---
ORDER = "viewCount"             # sort discovered videos by lifetime viewCount
EVENT_TYPE = None  # IMPORTANT: only use for live; None includes normal uploads
            ...
            if resp.status_code >= 400:
                ...
                if (resp.status_code in (403, 429)) and (reason in quota_reasons or (message and any(q in message for q in quota_reasons))):
                    ...
                    if rotate_api_key():
                        params["key"] = get_api_key()
                        continue
                    if CONTINUE_ON_QUOTA_ERRORS:
                        # No more keys; let caller truncate this query gracefully.
                        return {}
                raise RuntimeError(f"YouTube API {resp.status_code}: reason={reason} message={message}")
            try:
                return resp.json()
            except ValueError:
                # Sometimes the API returns a non-JSON body (e.g., HTML) even with 200/4xx.
                # Return an empty dict so callers can truncate safely.
                return {}
    ...
    for lang in LANG_HINTS:
        ...
        while True:
            ...
            if not isinstance(data, dict) or "items" not in data:
                print(f"[search] Empty/invalid response for '{q}' (lang={lang}); truncating this query.")
                break
...
# NOTE: Matching is performed on RAW title/description/tags (NFKC + lowercased, punctuation preserved),
# so contiguous patterns (e.g., \bIVE\b) will NOT match punctuated forms like "I've" or "I,VE".
# Multi-word aliases allow whitespace only between tokens.
...
def discover_for_day(T: date, kw_map: Dict[str, List[str]]) -> pd.DataFrame:
    # Only main brand tokens for matching (no aliases)
        ...
        for main_kw in alias_patts.keys():
            ...
            for h in hits:
                ...
                if alias_patts[main_kw].search(text_norm):
                    ...
                    # Strict UTC date match: drop items outside the exact day window returned by search
                    if pub_dt != day:
...
def main():
    print(f"[api-key] Starting with key #1/{len(API_KEYS)}")
